...
class AzureHttpRouter extends core_1.AbstractHttpAdapter {
    ...
    handle(context, request) {
        ...
        const path = originalUrl.slice(originalUrl.indexOf(hostname) + hostname.length);
        const { params, handlers } = this.instance.find(req.method, path);
        req.params = params;
        if (handlers.length === 0) {
            return this.handleNotFound(context, req.method, originalUrl);
        }
        const azureRequest = new azure_request_1.AzureRequest(context);
        const azureReply = new azure_reply_1.AzureReply(context);
